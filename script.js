document.addEventListener('DOMContentLoaded', () => {
    ...
    // Moved function definitions *BEFORE* the entry screen listener that calls them
    ...
    function generateRandomCandleData(previousClose) {
        ...
        let open = validPrevClose + randFactor(2, -2); let close = open + randFactor(8, -8);
        open = Math.max(chartConfig.priceMin * 0.8, Math.min(chartConfig.priceMax * 1.2, open)); close = Math.max(chartConfig.priceMin * 0.8, Math.min(chartConfig.priceMax * 1.2, close));
        const high = Math.max(open, close) + randFactor(3); const low = Math.min(open, close) - randFactor(3);
        const time = Math.floor(Date.now() / 1000);
        return { time, open, high, low, close };
    }

    function initializeChart(container, dataArrayRef) {
        if (!container || typeof LightweightCharts === 'undefined') { console.error("Chart container or LightweightCharts library not found for: ", container?.id); return null; }
        container.innerHTML = ''; // Clear previous chart if any
        const chartOptions = { width: container.clientWidth || 220, height: container.clientHeight || 140, layout: { background: { color: chartConfig.chartBackgroundColor }, textColor: chartConfig.textColor, }, grid: { vertLines: { visible: false }, horzLines: { visible: false }, }, crosshair: { mode: LightweightCharts.CrosshairMode.Hidden }, timeScale: { visible: false, fixLeftEdge: true, fixRightEdge: true, lockVisibleTimeRangeOnResize: true, borderVisible: false }, priceScale: { visible: false, borderVisible: false }, handleScroll: false, handleScale: false, };
    ...
    function updateChart(chartInfo, dataArray) {
        ...
        const lastCandle = dataArray[dataArray.length - 1]; const nextCandle = generateRandomCandleData(lastCandle.close); nextCandle.time = lastCandle.time + 300;
        chartInfo.candleSeries.update(nextCandle); dataArray.push(nextCandle);
        if (dataArray.length > chartConfig.candlesToShow * 2) { dataArray.shift(); }
        chartInfo.chart.timeScale().scrollToPosition(-1, false); // Scroll to latest
    }
    ...
    function startStockChartAnimation() {
        ...
        const initLeft = initializeChart(chartContainerLeft, currentDataLeft); if (initLeft) { chartLeft = initLeft.chart; candleSeriesLeft = initLeft.candleSeries; stockIntervalLeft = setInterval(() => updateChart(initLeft, currentDataLeft), chartConfig.updateInterval); }
        const initRight = initializeChart(chartContainerRight, currentDataRight); if (initRight) { chartRight = initRight.chart; candleSeriesRight = initRight.candleSeries; setTimeout(() => { stockIntervalRight = setInterval(() => updateChart(initRight, currentDataRight), chartConfig.updateInterval + 30); }, 500); }
